...
public class TemplateTreeController implements Serializable {
	...
	/**
	 * When some node is selected, we can verify selected type, current group
	 * and current section. Then it's more clear, what we are working with at
	 * the moment.
	 */
	public void onSelect() {
		...
		if (selectedType.equals("question")) {
			...
			GroupBean groupBean = (GroupBean) selectedNode.getParent()
					.getData();
			if (currentGroup != groupBean) {
				currentGroup = groupBean;
			}
			// check if current section has not been changed
			SectionBean sectionBean = (SectionBean) selectedNode.getParent()
					.getParent().getData();
	...
	public String addGroup() {
		...
		groupBean.setGroupName("GROUP_"
				+ (currentSection.getGroupsList().size() + 1));
	...
	public void addGroupToCurrentSection(GroupBean groupBean) {
		if (currentSection != null
				&& !currentSection.getGroupsList().contains(groupBean)) {
	...
	public long getNextQuestionIdInCurrentGroup() {
		if (currentGroup.getQuestionsList().isEmpty()) {
			...
			return ((currentGroup.getQuestionsList().get(currentGroup
					.getQuestionsList().size() - 1)).getId() + 1);
		}
	}
