...
When the user fills 'formTemplate:name' field with 'LQE-17 test'
...
Scenario:
Narrative:
User can select "FILEUPLOAD" in the Question
Type drop-down list.
...
When the user fills 'form2:formWithCommonProps:qtext' field with 'Test text'
Then wait until all animations on page completed
When the user fills 'form2:formWithCommonProps:helpText' field with 'Test text'
Then wait until all animations on page completed
When clicks on element with id/name/className 'ui-chkbox-box'
Then wait until all animations on page completed
Scenario:
Narrative:
User can save the
template with added
questions.
When the user clicks on element with id/name/className 'form2:formWithSaveButtons:savequestion'
Then wait until all animations on page completed
When in tree 'formTemplate:form:treeMultiple' user opens node with 'Page 1'
